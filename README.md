...
| `NUXT_GSI_RECORD` | путь к файлу записи сырых пакетов (пусто — запись выключена) |
| `NUXT_RECORDINGS_DIR` | каталог, где пульт ищет записи для воспроизведения |
...
- **Воспроизведение записи** — сохранённая GSI-сессия проигрывается в
  реальном темпе, со своими паузами. Управление на той же странице пульта:
  выбор файла, пауза, перемотка, скорость до десятикратной. Темп отсчитывает
  сервер, потому что таймеры в неактивной вкладке браузер душит.

  Перемотка начинает с чистого состояния матча: события из пропущенного
  куска не срабатывают. Иначе прыжок через десять минут выдал бы залп из
  смерти, трёх уровней и аганима разом.
